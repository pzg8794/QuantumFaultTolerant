...
\caption{Main stochastic-regime performance summary. The exported validation figure shows algorithm-tier separation under stochastic decoherence, complementing the RQ1 evidence table.}
...
\caption{Robustness floor under structured and adaptive threats. The exported validation figure shows which bandit policies maintain stable worst-case performance and which become fragile under adaptive disruption.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/icnp/ICNP-CODE-039_g1_capacity_paradox.png}
\caption{Replay capacity has threat-dependent effects. The exported validation figure shows where additional replay scale improves efficiency and where it reduces robustness under adaptive disruption.}
% \shee{Can data labels be added to all graphs?} -- SOLVED: Replaced temporary PGFPlots placeholder with exported notebook-validated image containing the finalized graph styling/labels.
...
\caption{Scenario-based deployment guidance. The exported validation figure summarizes when the strong static default is sufficient and when threat-adaptive switching improves robustness.}
