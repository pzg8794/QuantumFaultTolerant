...
\caption{Worst-case robustness floor under structured and adaptive threats. The figure reports each representative policy's lowest Oracle-normalized efficiency across the locked adversarial scope. Higher floors indicate policies that avoid catastrophic collapse; the EXPNeuralUCB floor highlights that adversarial-style exploration alone is not sufficient for stable quantum-routing performance.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/icnp/ICNP-CODE-042_g4_deployment_rules.png}
\caption{Scenario-based deployment guidance for iCPursuitNeuralUCB. The figure compares the strong static default against scenario-specific switching rules, showing that switching yields modest gains overall but is most useful under adaptive disruption, where allocator/capacity mismatch has the highest risk.}
