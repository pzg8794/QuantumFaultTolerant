...
\centering
\fbox{%
  \begin{minipage}[c][1.35in][c]{0.94\columnwidth}
  \centering
  \textit{Placeholder: main performance summary graph}\\[0.4em]
  Compact grouped/stacked summary of representative bandit policies\\
  y-axis: Oracle-normalized efficiency (\%)\\
  highlight viable, degraded, and collapsed policy tiers
  \end{minipage}
}
\caption{Main stochastic-regime performance summary. The final graph should show which bandit policies remain viable under stochastic disruption and which collapse, complementing the RQ1 evidence table.}
\label{fig:main_performance_summary_placeholder}
\end{figure}
...
\centering
\fbox{%
  \begin{minipage}[c][1.40in][c]{0.94\columnwidth}
  \centering
  \textit{Placeholder: robustness floor graph}\\[0.4em]
  Bar or lollipop chart by representative bandit policy\\
  y-axis: worst-case efficiency floor (\%)\\
  include 85\% target reference line and adaptive-threat scope
  \end{minipage}
}
\caption{Robustness floor under structured and adaptive threats. The final graph should show which bandit policies maintain stable worst-case performance and which become fragile under adaptive disruption.}
...
\centering
\fbox{%
  \begin{minipage}[c][1.55in][c]{0.94\columnwidth}
  \centering
  \textit{Placeholder: capacity paradox graph}\\[0.4em]
  Diverging bar chart by threat scenario\\
  y-axis: replay-capacity effect in percentage points\\
  include zero baseline; positive = helps, negative = hurts
  \end{minipage}
}
\caption{Replay capacity has threat-dependent effects. The final graph should show where additional capacity improves efficiency and where it reduces robustness under adaptive disruption.}
...
\fbox{%
  \begin{minipage}[c][1.40in][c]{0.94\columnwidth}
  \centering
  \textit{Placeholder: deployment guidance graph}\\[0.4em]
  Scenario-to-configuration rule map or compact decision table\\
  show static default plus threat-adaptive switching gains\\
  emphasize Adaptive as largest switching benefit
  \end{minipage}
}
\caption{Scenario-based deployment guidance. The final graph should summarize when the strong static default is sufficient and when threat-adaptive switching improves robustness.}
\label{fig:deployment_guidance_placeholder}
\end{figure}
