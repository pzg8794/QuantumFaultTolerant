...
\end{tikzpicture}
\caption{Four-path quantum-network testbed. The topology provides two shorter
two-hop paths ($P_1$, $P_2$) and two longer three-hop paths ($P_3$, $P_4$),
creating the path-length and qubit-allocation tradeoff evaluated throughout the
study.}
...
\caption{Matched evaluation-grid design matrix. This matrix maps each
configuration axis---topology, horizon, qubit capacity, allocator, replay
semantics, threat model, forecasting, and policy family---to its tested options
and research-question coverage. It supports the study-design claim that routing
robustness is evaluated under matched configuration factors rather than isolated
experiments, enabling attribution of performance differences to the
policy--allocator--capacity--threat interaction.}
...
\centering
\setlength{\tabcolsep}{4pt}
\caption{Matched hyperparameter specification for reproducible policy comparison.
This specification maps each implementation-sensitive parameter---neural learning
rate, exponential weighting, pursuit update rate, exploration constant,
forecasting window, and replay semantics---to its fixed value and literature or
tuning source. It supports the study-design claim that policy performance is
compared under a controlled implementation layer, enabling robustness differences
to be attributed to policy family, allocator choice, replay configuration, and
threat regime rather than hidden hyperparameter variation.}
...
\centering
\setlength{\tabcolsep}{4pt}
\caption{Matched threat-scenario specification. This specification maps each
scenario---Baseline, Stochastic, Markov, Adaptive, and OnlineAdaptive---to its
disruption process and research-question coverage. It supports the
matched-threat evaluation claim that routing policies are tested along a
controlled escalation from no-disruption operation to stochastic, structured,
and adaptive interference, enabling robustness differences to be attributed to
policy behavior under increasingly strategic disruption.}
...
\centering
\setlength{\tabcolsep}{4pt}
\caption{Policy-family coverage for the matched robustness comparison. This
portfolio defines the learning-design axis evaluated in the study: classical
MAB, adversarial, contextual/neural, pursuit-based, predictive, and
Oracle-reference policies. It supports the study-design claim that later
robustness comparisons are family-level comparisons made through a common
routing/reward interface, with the Oracle providing a shared reference across
evaluation phases.}
...
\centering
\setlength{\tabcolsep}{4pt}
\caption{Qubit allocation strategies as deployment controls. This table compares
the allocator mechanisms used in the matched evaluation grid and reports their
average efficiency across scenarios, showing that allocator choice changes
routing performance under the same policy/reward interface. The observed spread
from \texttt{Thompson} at 88.2\% to \texttt{Random} at 77.3\% supports the
study-design claim that allocator policy is a first-class factor in the
policy--allocator--capacity--threat interaction.}
