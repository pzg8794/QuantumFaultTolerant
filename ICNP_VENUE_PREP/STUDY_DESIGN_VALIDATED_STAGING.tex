...
\caption{Matched evaluation-grid design matrix. Configuration axes, tested
options, and RQ coverage define the controlled
policy--allocator--capacity--threat comparison.}
...
\centering
\setlength{\tabcolsep}{4pt}
\caption{Matched hyperparameter specification. Fixed, sourced settings keep
robustness comparisons attributable to policy family, allocator, replay, and
threat regime.}
...
\centering
\setlength{\tabcolsep}{4pt}
\caption{Matched threat-scenario specification. Baseline through
OnlineAdaptive define the controlled escalation used for RQ1/RQ2 robustness
attribution.}
...
\centering
\setlength{\tabcolsep}{4pt}
\caption{Policy-family coverage for matched robustness comparison. Classical,
adversarial, contextual/neural, pursuit-based, predictive, and Oracle-reference
policies share one routing/reward interface.}
...
\centering
\setlength{\tabcolsep}{4pt}
\caption{Qubit allocation strategies as deployment controls. The
\texttt{Thompson}--\texttt{Random} efficiency spread shows allocator policy as
a first-class factor in the policy--capacity--threat interaction.}
