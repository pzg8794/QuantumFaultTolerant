...
We distinguish the base horizon $F_b\in\{4\mathrm{K},6\mathrm{K},8\mathrm{K}\}$ from the current horizon $F_c$ used after frame scaling, and evaluate $S\in\{3,5,8,10\}$ \shee{what is S?} independent runs per configuration. Replay memory is scaled by $s\in\{1,1.5,2\}$, with default $s=2$, under two semantics:
\[
...
CPursuitNeuralUCB adds topology/channel-quality context to pursuit learning, while iCPursuitNeuralUCB adds ARIMA$(1,0,1)$ forecasts with $n\in\{50,100\}$ warmup~\cite{box2015time} \shee{Define n}. The Oracle normalizes efficiency to a perfect-information reference.
