...
\node[mainbox, below=1.4cm of threats] (engine) {Threat-Aware Evaluation Framework\\Joint path selection + qubit allocation under matched conditions};
...
\begin{tikzpicture}[
  ...
  \draw[link] (R2) -- (D);
  \draw[link] (R1) -- (R2);
  \draw[path, blue] (S) to[bend left=15] node[above, font=\scriptsize] {$P_1$ (2-hop)} (R1) to[bend left=15] (D);
  \draw[path, red] (S) to[bend right=15] node[left, font=\scriptsize] {$P_2$ (2-hop)} (R2) to[bend right=15] (D);
  \draw[path, green!60!black] (S) to[bend left=10] (R1) to[bend left=10] node[right, font=\scriptsize] {$P_3$ (3-hop)} (R2) to[bend left=10] (D);
  \draw[path, orange] (S) to[bend right=10] (R2) to[bend right=10] node[left, font=\scriptsize] {$P_4$ (3-hop)} (R1) to[bend right=10] (D);
\end{tikzpicture}
...
\begin{tabular}{>{\raggedright\arraybackslash}p{0.28\linewidth} >{\raggedright\arraybackslash}p{0.50\linewidth} >{\centering\arraybackslash}p{0.12\linewidth}}
\toprule
