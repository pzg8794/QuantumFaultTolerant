...
\shee{I feel this list might be redundant since the details are present in Fig 3 and explainged in the next paragraph.}
\begin{enumerate}
...
\end{tikzpicture}
% \vspace{-0.5cm}
\caption{Pursuit-based hybrids maintain higher efficiency than neural and EXP3 baselines across all five threat regimes.\shee{This legend is not self-explanatory. The graph has 2 colored portions visible, but the legend has 5 entries with no colors. Try a different type of chart. If an area chart is the best way to show this graph, it might be better to show each model in its own graph and arrange them together. Same applies to Fig 10 as well}}
