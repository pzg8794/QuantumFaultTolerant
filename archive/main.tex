...
The canonical internal topology operates under a \textbf{35-qubit baseline budget} distributed across paths. This 35-qubit setting defines the controlled internal system model in this section, not a global limit of the broader framework, which also supports topology-specific budgets for the external testbeds. We evaluate \textbf{four allocator strategies} chosen to span complementary allocation behaviors: a static baseline (\textbf{Fixed}), two principled adaptive heuristics based on posterior sampling and optimistic exploration (\textbf{ThompsonSampling} and \textbf{DynamicUCB}), and a non-informative control baseline (\textbf{Random}). This set allows us to compare whether routing robustness is best supported by fixed, uncertainty-aware, or uninformed allocation under the same total resource budget.
\begin{enumerate}
...
\end{tikzpicture}
% \vspace{-0.5cm}
\caption{Contextual (CMAB/iCMAB) methods are more scale-robust than EXP3 baselines, with the clearest Oracle-normalized efficiency gap under $T_b$ settings.\devroop{You may want to zoom in for this figure to show the exact curve variations}}
...
For the canonical internal topology, total physical network capacity is fixed at 35 qubits, representing a resource-constrained baseline deployment~\cite{simon2017towards}. This is not a framework-wide ceiling: the broader evaluation also includes external testbeds with topology-specific native budgets. The fixed 35-qubit baseline induces non-trivial exploration--exploitation tradeoffs: algorithms cannot over-provision all paths. Per-path allocations are determined by the allocator (\Cref{tab:setup-allocators}), enabling controlled comparison of static versus adaptive resource management.
...
\end{tikzpicture}
% \vspace{-0.5cm}
\caption{More capacity helps in Baseline and Markov settings but can hurt under Adaptive threats, revealing the capacity paradox.\devroop{You may want to zoom in for this figure to show the exact curve variations}}
...
\end{tikzpicture}
% \vspace{-0.5cm}
\caption{Allocator choice is threat-dependent: Thompson leads in Adaptive regimes, DynamicUCB stays competitive, and Random underperforms.\devroop{You may want to zoom in for this figure to show the exact curve variations}}
...
To validate the generalizability and robustness of our proposed algorithms, we conducted cross-testbed evaluations by deploying our model portfolio on four independent quantum network simulation testbeds from prior work. These testbeds vary significantly in scale, network topology, noise modeling, and fidelity calculation approaches, providing a rigorous stress test for our context-aware pursuit-neural algorithms. This section presents: (1) performance comparison across the four external testbeds (\paperTwo, \paperSeven, \paperTwelve, and \paperEight), and (2) a consolidated comparison of model family performance (CMABs, iCMABs, Hybrid pursuit-neural, and EXP3-based models) across our internal evaluation suite.
...
\paperTwo, \paperSeven, and \paperTwelve use the full multi-scale, 5-run evaluation corpora summarized above, while \paperEight is currently represented by its original paper-config slice (1K/1K/1R, $s=1.0$, \texttt{cap\_type}=T).
...
\item \textbf{\texttt{iCPursuitNeuralUCB} is the cross-layer winner on \paperTwo, \paperSeven, and \paperTwelve, while on \paperEight it dominates only.} It wins 95/300 experiments on \paperTwo, 245/300 on \paperSeven (81.7\%), and 97/300 on \paperTwelve. On \paperSeven, its dominance is overwhelming---no other model comes close. On \paperTwo and \paperTwelve, the margin is tighter: \paperTwo sees a 4-way race (iCP: 95, G: 84, EXP: 83, CP: 38), while \paperTwelve is a 2-way race (iCP: 97, EXP: 91). \paperEight breaks this pattern: \texttt{EXPNeuralUCB} wins 10/20 allocator-scenario configurations, while \texttt{iCPursuitNeuralUCB} leads only 1/20 despite having the strongest aggregated efficiency.

\item \textbf{Experiment threat ranking does not fully determine experiment-level dominance.} In the \paperTwo testbed, \texttt{iCPursuitNeuralUCB} has the highest dominance (scenario-aggregated efficiency, 74.45\%) yet wins only 2/5 threat scenarios (none, markov). \texttt{CPursuitNeuralUCB} and \texttt{GNeuralUCB} each win 3/5 threat scenarios despite lower dominance, capturing adaptive, onlineadaptive, and stochastic. The \paperEight testbed shows the same separation in a different form: \texttt{iCPursuitNeuralUCB} has the highest dominance (scenario-aggregated efficiency, 67.85\%), while \texttt{EXPNeuralUCB} wins the largest share of allocator--threat-scenario configurations (10/20).

\item \textbf{The \paperTwelve testbed exhibits the broadest overlap in threat-scenario winners.} All four models win in at least 4 of 5 threat scenarios, with \texttt{markov} uniquely favoring \texttt{iCPursuitNeuralUCB} (51/60 experiment wins). This contrasts with \paperSeven where threat-scenario championship is concentrated: \texttt{iCPursuitNeuralUCB} wins all 5 threat scenarios, with only \texttt{GNeuralUCB} and \texttt{EXPNeuralUCB} sharing \texttt{markov} experiment wins.

\item \textbf{The \paperEight testbed adds a compact RL-routing regime with competitive but lower aggregate efficiencies than \paperTwo and \paperSeven.} Its model efficiencies span 61.4--67.9\%, placing it below \paperTwo and \paperSeven but well above \paperTwelve. This extends the testbed (default allocator) winner story: the compact 20-node, 8-path \paperEight network splits \texttt{EXPNeuralUCB} experiment wins from \texttt{iCPursuitNeuralUCB} dominance, unlike the strong \texttt{iCPursuitNeuralUCB} cross-layer-winner pattern seen on \paperSeven.

\item \textbf{\texttt{CPursuitNeuralUCB} never wins a threat scenario on \paperSeven yet wins 3/5 threat scenarios on \paperTwo.} This testbed specificity highlights that winner structure is topology-dependent: the dense 50-node \paperSeven network strongly favors \texttt{iCPursuitNeuralUCB} as the cross-layer winner, while the 15-node \paperTwo network enables broader competition.
\end{enumerate}

\subsection{Model Family Performance Comparison}

To provide a consolidated view of algorithm performance across model families, we aggregated results from our internal evaluation suite spanning CMABs, iCMABs, Hybrid pursuit-neural, and EXP3-based models. \Cref{tab:model_family_comparison} presents representative algorithms from each family under controlled conditions (4K base frames, 2K step frames, 5 runs, \texttt{cap\_type}=Tb), aggregated across all five scenarios using the \texttt{Default} allocator and available capacity scales. To enable direct comparison, we also include hybrid pursuit-neural results from the four external testbeds (\paperTwo, \paperSeven, \paperTwelve, and \paperEight) filtered to the same \texttt{Default} allocator (\texttt{cap\_type}=T, the only capacity semantics available in those corpora).
...
\item \textbf{\texttt{iCPursuitNeuralUCB} is the strongest neural model by Oracle-relative gap reduction.} Across the Hybrid corpus and all four external testbeds (\paperTwo, \paperSeven, \paperTwelve, and \paperEight), it achieves the lowest aggregate gap over the five scenarios, and it retains that lead within each allocator slice. Its leadership is therefore stable to both topology and allocator variation.

\item \textbf{Efficiency degrades with testbed complexity.} Hybrid performance drops from 90.9\% on the internal corpus to 74.4\% (\paperTwo, 15N), 77.5\% (\paperSeven, 50N), and 41.6\% (\paperTwelve, 100N). Crucially, low efficiency on the \paperTwelve testbed is \emph{not} caused by an intrinsic fidelity ceiling: the Oracle achieves an average reward of 0.87 ($\approx$87\% of the theoretical maximum), and individual model configurations reach up to 63\% of Oracle. The gap instead reflects the difficulty of bandit optimization on a 100-node, 4-path fusion topology---the large state space and constrained routing diversity make arm selection harder to learn within the 1.5K/500 frame regime, whereas the internal 4K/2K regime on a smaller topology gives the learner sufficient time to converge.
...
The cross-testbed validation presented in this section uses each testbed's native paper-config regime (\paperTwo: 4K/2K, \paperSeven: 50/50, \paperTwelve: 1.5K/500, \paperEight: 1K/1K/1R) while aggregating across allocators and scales. While this validates external generalizability under each testbed's original evaluation setup, it does not yet isolate differences under shared standardized run configurations. To address this, we are conducting a follow-up evaluation campaign that deploys all four external testbeds (\paperTwo, \paperSeven, \paperTwelve, and \paperEight) using our \textbf{standardized run configurations}:
...
\item \textbf{Topology-algorithm interaction analysis} to determine whether algorithm rankings shift across sparse (\paperTwo: 15N, 51E), dense (\paperSeven: 50N, 141E), and mid-scale (\paperTwelve: 100N, 426E) network structures
\end{enumerate}
...
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\paperSeven}\\ (\small 50N, 141E, 15P)\\ \small 4K/2K/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
...
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\paperTwelve}\\ (\small 100N, 426E, 4P)\\ \small 4K/2K/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
...
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\paperEight}\\ (\small 20N, 19E, 8P)\\ \small 4K/2K/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
...
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\paperTwo}\\ \small(Standard)\\ \small 4K/2K}}
& \texttt{iCPursuitNeuralUCB} & \textbf{74.44} & 25.56 & 54.4 & \textbf{25/75$^\star$} \\
& \texttt{CPursuitNeuralUCB} & 73.15 & 26.85 & 48.3 & 8/75 \\
& \texttt{GNeuralUCB} & 73.17 & 26.83 & 46.7 & 20/75 \\
& \texttt{EXPNeuralUCB} & 71.29 & 28.71 & 41.6 & 22/75 \\
...
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\paperSeven}\\ \small(Standard)\\ \small 4K/2K}}
...
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\paperTwelve}\\ \small(Standard)\\ \small 4K/2K}}
...
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\paperEight}\\ \small(Standard)\\ \small 4K/2K}}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/icnp_graphs/G7_advanced_synthesis.png}
\caption{A four-panel synthesis shows the paper's core mechanism: hybrid policies shrink Oracle gaps, replay scaling has threat-dependent effects, allocator choice controls tail risk, and standardized testbeds expose topology-dependent efficiency spread.}
\label{fig:advanced_synthesis}
\end{figure*}
...
As outlined in \cref{sec:testbed_comparison}, we are conducting follow-up evaluations that deploy all four external testbeds (\paperTwo, \paperSeven, \paperTwelve, and \paperEight) using our standardized run configurations (4K base/2K step frames, 3/5/10 runs, unified threat scenarios). This will enable direct efficiency comparisons across sparse (15-node), compact (20-node), dense (50-node), and mid-scale (100-node) topologies under aligned experimental conditions, testing whether pursuit-neural cross-layer winning structure persists and whether the capacity paradox manifests consistently across network structures.
