...
| C-061 | 2026-03-13 | Results (TABLE VI) | Internal audit: `tab:rq2_adversarial` used `Win Share (%)` without a defensible denominator in the manuscript caption or source-backed validation flow | Replace the unsupported winner-frequency column with source-backed `Win Dominance (%)`; define the denominator explicitly in the caption and update the four values from the validation hub | Done | TBD | T-2026-007, T-2025-011 | Applied in `main.tex` at `\label{tab:rq2_adversarial}`. `Win Dominance (\%)` is now defined as each model's share of aggregated scenario wins among the four displayed representatives under the locked RQ2 adversarial scope; values updated to `25.6 / 43.9 / 30.5 / 0.0`. |
| C-062 | 2026-03-13 | Results (RQ3a / TABLE VII) | Internal audit: `tab:rq3a_informative` caption-faithful reconstruction (`6K`, `Fixed`, `T`, `s=2`, mean of 3/5 runs) did not support the manuscript OA-lift or `CV_scen` claims | Correct RQ3a to the strongest source-backed provenance branch (`6K`, `Fixed`, `T`, `s=2`, 3-run suite), update the two table rows, and make the tighter-dispersion claim explicit with the validated `CV_scen` values | Done | TBD | T-2026-007, T-2025-011 | Applied in `main.tex` at `\label{tab:rq3a_informative}`. Scope text and caption now use the 3-run suite, table values updated to `99.8/94.7/93.0/92.8/81.5/92.4/6.5` and `99.9/94.8/93.0/92.8/99.8/96.0/3.3`, and the RQ3a answer now states `CV_scen: 6.5 -> 3.3` while preserving the validated `+18.3 pp` OnlineAdaptive lift. |
...
- `2026-03-31` — `R-13` was promoted from a single-table caption fix to the governing style for the manuscript-wide table pass.
- Applied a full table-caption sweep in `main.tex` using the `RQ2` adversarial table as the reference pattern: takeaway first, compressed context second, and no setup-heavy aggregation recipe in the caption unless essential.
- Normalized table sizing directives and note/legend style for consistency across setup, results, and cross-testbed tables without changing scientific claims or numerical values.
- Added a separate later-review note to audit the remaining non-`RQ2` hypothesis subsections for possible removal or compression.
...
| Technical/Engineering | T-004 | 2026-03-01 | Framework / State Naming | Random allocator state naming includes qubit allocations in runner/model filenames | Drop qubit-allocation suffix from **runner/model** state filenames for Random allocator; allow canonical overwrite (day folder still preserves history) | Done | 9304ad83 | T-2025-011 | Implemented in framework repo (`experiment_runner.py`, `base_bandit.py`) |
| Technical/Engineering | T-005 | 2026-03-01 | Framework / Resume Logic | Resume path relies on parsing `_(qubit_alloc)` in runner filenames | Make Random-allocator resume allocation-agnostic; keep backward compatibility by matching stems against legacy `_(d_d_d_d)` filenames and selecting the largest file | Done | 9304ad83 | T-2025-011 | Implemented in framework repo (`experiment_config.py`, `experiment_runner.py`) |
| Technical/Engineering | T-006 | 2026-03-01 | Framework / Evaluator Audit | Ensure evaluator naming + resume behavior matches the intended policy (Random allocator only) | Audit `MultiRunEvaluator` + `ExperimentConfiguration.get_key_attrs()` + `LocalBackupManager` registry handling to ensure (i) evaluator filenames are allocation-agnostic, (ii) evaluator resume/equality does not require qubit allocations in names, and (iii) **for Random allocator only** we intentionally ignore `qubit_capacities` in evaluator comparisons while still requiring runner instances to store the exact allocation used per experiment for apples-to-apples model comparison | Done | 9304ad83, 538188e7, 99743eba, ad608f2a, d7a2b1a6, 61108f28, 723e6d6a, 4184f2bd | T-2025-011 | Implemented safe state aggregation + registry rebuild (`tools/state/aggregate_state_dirs.py`), integrated into `AllocatorRunner` (default ON; toggle via `DAQR_AGGREGATE_STATE=0` or `framework_config['aggregate_state']=False`), and enforced a deterministic duplicate filename policy in `LocalBackupManager` scan (largest wins; mtime breaks ties). Added small “no full runs” test battery + runner script: `bash Dynamic_Routing_Eval_Framework/tools/tests/run_small_tests.sh`. Also removed duplicate `run_scripts/` (kept `scripts/`). |
...
| Review/Submission | T-010 | 2026-03-30 | Tables (global consistency pass) | Table captions and table formatting were not yet stylistically uniform across the manuscript | Run one formatting-only sweep covering table caption tone/length, sizing directives, column-header style, label/title style, and notation consistency (`T` vs `T_b`, scale notation, Oracle-normalized wording); do not broaden this into numerical/result rewrites | Done |  | T-2025-011 | Pulled into active work by user request and merged with `R-13` as the governing all-table caption/style lane |
| Review/Submission | T-011 | 2026-03-31 | Hypotheses (remaining audit) | After the flagged `RQ2` hypothesis removal, the remaining hypothesis subsections should be reviewed for possible removal or compression | Audit the non-`RQ2` hypothesis subsections one by one and decide whether each should stay, be shortened, or be removed for space/clarity | Deferred |  | T-2025-011 | Separate from the table pass; do not activate unless explicitly pulled in |
...
| Technical/Engineering | T-008 | 2026-03-06 | Framework / Analysis Tooling | `state_analysis.py` log output is misleading for (i) fallback unpickle success and (ii) recoverable extraction warnings | Update logging messages to clearly distinguish: `Loaded successfully on attempt 2/3` (cloudpickle/SafeUnpickler), `Extraction warning (recovered)` when rows were still extracted, and `Skipping (no evaluation payload)` for expected base artifacts | Done |  | T-2025-011, T-2026-012 | Also record corpus naming rule: “Hybrid” = internal catch-all for state files that do **not** end in an explicit corpus suffix pattern (e.g., EXP3/iCMABs). |
| Technical/Engineering | T-009 | 2026-03-06 | Framework / Analysis Contract | `state_analysis.py` and current evaluator states diverged; consumer contract is no longer documented explicitly | Record the expected evaluator payload contract, document the current observed state shape (`env_experiments['n/a']` without matching `scenarios_results['n/a']`), and use the existing Hybrid/EXP3/iCMABs + paper2/7/12 master datasets as regression references before any code fix | In Progress |  | T-2025-011, T-2026-012 | Canonical reference: `hybrid_variable_framework/docs/guides/STATE_ANALYSIS_EVALUATOR_CONTRACT.md`. Working diagnosis on 2026-03-06: current Paper 12 and Paper 8 states both expose `n/a` in `env_experiments`; extraction success/failure currently depends on scenario iteration order rather than a stable payload contract. |
