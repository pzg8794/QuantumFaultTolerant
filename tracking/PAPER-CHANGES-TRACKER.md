...
| C-007 | 2026-02-15 | References + Cross-Testbed Validation | Paper 12 / QuARC misattributed as “Wang et al.” (but QuARC paper is Clayton/Wu/Bhattacharjee) | After approval: add a dedicated BibTeX key (e.g., `clayton2024quarc`) for QuARC (arXiv:2410.23007) and update any Paper 12 testbed citations/mappings to point to it; keep `wang2024adaptive` reserved for Lei Wang’s user-centric routing paper | Planned |  | T-2026-007, T-2025-015 | Verified from the QuARC PDF author list (Connor Clayton et al.) vs the “Adaptive User-Centric…” PDF (Lei Wang et al.) |
...
| **Quantum Network Routing with Bandits** | Drafted (not applied) | Drafted explicit Huang attribution + LinkSelFiE contrast + grouped comparisons (learning/RL/heuristic routing + benchmarking frameworks) via P-001/P-002/P-003 |
...
- Keep duplicate/placeholder BibTeX-key cleanup as a *separate* review item (do not edit `refs.bib` while reviewing Related Work wording), because BibTeX-key issues can change numbering and create noise while validating the prose.
- Cross-testbed attribution sanity check: Paper 12 (QuARC / arXiv:2410.23007) is authored by **Connor Clayton et al.**, so any BibTeX entry/citation that labels it “Wang et al.” should be corrected as part of C-007 after approval.
...
These are **draft insertions** prepared for review. **Current state:** draft-only (not applied to the manuscript) until explicitly approved.
...
```tex
Huang et al.~\cite{huang2024quantum} propose \emph{EXPNeuralUCB}, a group neural bandit that combines EXP3-style adversarial exploration with NeuralUCB-style nonlinear reward modeling for joint path selection and qubit allocation. We treat EXPNeuralUCB as a key adversarial/hybrid baseline and show that our pursuit--neural hybrids achieve higher scenario-aggregated efficiency on the external testbeds, while our framework treats allocator strategy, replay capacity, and threat type as first-class experimental variables rather than fixed implementation choices.
```
...
```tex
Beyond bandit-style path selection, learning-based route selection under noisy quantum-network conditions has also been explored~\cite{chaudhary2023quantum}, and RL-based adaptive routing has been proposed via deep Q-networks~\cite{jallowkhan2025adaptive}. Complementary non-learning routing designs emphasize structural decomposition (e.g., QuARC adaptive clustering~\cite{clayton2024quarc} and hierarchical routing for scalability~\cite{cicconetti2024scalable}) or repeater/efficiency constraints~\cite{kumar2024routing}, while cost-vector approaches optimize multi-path routing decisions through explicit objective formulations~\cite{leone2021costvector}. In contrast, our contribution is a controlled evaluation methodology that isolates how decision-rule families interact with allocation policies, replay semantics, and capacity across a shared threat taxonomy, enabling direct attribution of robustness to the algorithm--allocator--capacity triad rather than to a single routing primitive.
```
...
**Proposed fix (LaTeX) — do not apply until approved:**
- `paper2_reference` → `chaudhary2023quantum`
- `paper7_reference` → `liu2024qbgp`
- `paper12_reference` → `clayton2024quarc`

**Where it would be applied (after approval):** [../main.tex](../main.tex), Cross-Testbed Validation section (external testbed configuration bullets + cross-testbed validation summary).

**Verification note (why `clayton2024quarc`):** The PDF currently tracked for “QuARC / Paper 12” (arXiv:2410.23007) lists authors as \textbf{Connor Clayton, Xiaodi Wu, and Bobby Bhattacharjee} (University of Maryland), not “Wang et al.”. This key rename (and/or BibTeX author correction) must be done in `refs.bib` in a follow-up step after approval.
