...
- `refs.bib` currently contains likely-duplicate keys/entries (e.g., `smith2024costvector` vs `leone2021costvector`, repeated `buchholz2023quantum`, and multiple utility-framework entries). This does not block C-003, but it can cause accidental double-citation or inconsistent attribution if not cleaned up later.

## Proposed Edits (Review Before Applying)

These are **draft insertions** prepared for review. **Current state:** the text for P-001/P-002/P-003/P-004 has been applied in the working tree (uncommitted) to `02--related_works.tex`, but should still be reviewed before finalizing/committing.
