...
| C-063 | 2026-04-26 | Cross-Testbed Validation | Devroop: reader-facing manuscript text should use the exact reference/testbed source rather than visible “Paper X” labels | Define reusable author-based testbed macros and replace reader-facing `Paper 2/7/12/8` labels across the cross-testbed prose, tables, captions, notes, and standardized-testbed discussion | Done | TBD | T-2026-007, T-2025-011 | Applied in `main.tex`: introduced `\ChaudharyTestbed`, `\LiuTestbed`, `\ClaytonTestbed`, and `\JallowKhanTestbed`; removed the live reviewer note from the compiled manuscript path; `Paper 8` is now mapped to `jallowkhan2025adaptive`. |
