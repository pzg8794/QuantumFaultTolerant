...
We validate robustness through cross-testbed evaluation on three external quantum network simulators, confirming consistent robustness trends across diverse topologies and noise models while exposing scale- and physics-dependent performance limitations.
...
Quantum routing differs fundamentally from classical packet switching because the underlying resource is \emph{entanglement}, not transferable data. Quantum states cannot be copied or amplified due to the no-cloning theorem~\cite{wootters1982single}, rendering standard store-and-forward buffering impossible. Instead, communication relies on establishing entanglement---a consumable resource constrained by memory coherence times and operation-induced fidelity loss---and then consuming that entanglement to support protocols such as teleportation~\cite{bennett1993teleporting}. Furthermore, entanglement swapping succeeds only with a non-unit probability~\cite{zukowski1993event}, making link availability stochastic rather than deterministic. Consequently, routing must jointly optimize success probability and fidelity under uncertainty that evolves over time~\cite{li2025multipath,wang2025learning,huang2024quantum}.

However, existing quantum routing research often evaluates algorithms under \emph{incompatible assumptions, metrics, or threat models}, complicating direct comparisons and obscuring deployment tradeoffs~\cite{liu2024qbgp,li2025multipath,chaudhary2023quantum}. Adversarial-first approaches (\eg EXP3-family and adversarial-adaptive designs~\cite{auer2002nonstochastic,zimmert2019optimal,huang2024quantum}) assume worst-case link behavior and prioritize robustness under attack, often sacrificing stochastic efficiency, whereas stochastic/contextual-first approaches (\eg contextual and informed CMAB/iCMAB variants~\cite{chu2011contextual,wang2025learning,kar2024icmab}) leverage predictive environmental structure for efficiency under natural noise but offer limited evidence under coordinated adaptive adversaries. Despite strong progress in both directions, prior evaluations still leave three matched-threat gaps insufficiently isolated: (i) it remains unclear which threat and noise regimes truly require topology/channel context versus when non-contextual policies suffice, (ii) adversarial-first and contextual-first methods are rarely compared under identical controlled threat scenarios, limiting causal conclusions, and (iii) allocator choice and replay/capacity semantics are often treated as implementation details rather than first-class factors, obscuring interaction effects and deployment-relevant tradeoffs.


\subsection{Our Approach and Evaluation Scope}

To close these evaluation gaps, we introduce a unified framework that systematically compares adversarial-first (EXP3-family), stochastic/contextual-first (CMAB/iCMAB-family), and hybrid pursuit--neural models (including EXPNeuralUCB~\cite{huang2024quantum}) under a shared threat taxonomy. The evaluation pipeline is illustrated in Fig.~\ref{fig:framework}. Our results show that context-aware pursuit--neural hybrids consistently achieve the best balance of robustness and efficiency, while replay capacity reveals a threat-dependent “capacity paradox” that can increase vulnerability under adaptive disruption.

Section~\ref{sec:studydesign} provides details on the experimental grid, including models, threat scenarios, allocator strategies, and replay-capacity semantics.
...
\item \textbf{Unified, reproducible benchmarking under a shared threat taxonomy:} We evaluate adversarial (EXP3-family), contextual (CMAB/iCMAB), and hybrid pursuit--neural bandit policies under a shared threat taxonomy, enabling direct comparison of robustness--efficiency tradeoffs~\cite{auer2002nonstochastic,huang2024quantum,chu2011contextual,kar2024icmab}.

\item \textbf{Cross-testbed validation at multiple scales:} We validate our algorithms on four external quantum network testbeds from prior work~\cite{chaudhary2023quantum,liu2024qbgp,clayton2024quarc}, spanning 15--100 nodes, 4--15 routing paths, and diverse evaluation settings (stochastic gate/channel errors, benchmarking-driven fidelity estimation and online path selection, fusion-based entanglement generation, and reinforcement-learning-driven entanglement routing). Using scenario-aggregated metrics across all five threats, model efficiencies span 69.6--78.0\% on Papers 2/7, 61.4--67.9\% on Paper 8, and 42.5--44.1\% on Paper 12, indicating strong topology/physics dependence and clear mid-scale fusion-network limitations (\S\ref{sec:testbed_comparison}).
...
Building on these motivations, we systematically evaluate how modeling choices (contextual, adversarial, predictive), allocator strategies, and capacity semantics jointly impact routing robustness across diverse threat regimes. Our study benchmarks \textbf{16 models} (15 learned policies plus Oracle) over \textbf{5 threat scenarios}, \textbf{4 allocator strategies}, and capacity scales $S \in \{1.0, 1.5, 2.0\}$, using 3--5 run ensembles. This design isolates the most impactful factors and supports actionable deployment guidance for quantum networks.
...
\vspace{-0.45cm}
\caption{Modular framework for quantum routing evaluation: shared
configuration defines topology, threat regime, capacity semantics,
and seeds; allocators and models plug into a common
evaluator/runner pipeline; the visualizer aggregates
...
\emph{Under stochastic decoherence, are baseline MABs deployment-viable?}
...
\end{tikzpicture}
\vspace{-0.5cm}
\caption{Context vs.\ EXP3 (non-hybrid): Contextual (CMAB/iCMAB) methods
are more scale-robust than EXP3 baselines across scenarios, with higher
average Oracle-normalized efficiency and a wider gap under
capacity-constrained ($T_b$) settings across scales $s \in \{1.0,1.5,2.0\}$.}
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{RQ1a:} \textit{What performance floor do baseline MABs reach under pure stochastic disruption?} \\
  ...
  \item \textbf{RQ1b:} \textit{Do topology/channel-aware models outperform baselines under the same disruption?} \\
  ...
  \item \textbf{RQ1c:} \textit{Do top-tier contextual MABs remain above the 85\% deployment target across horizons and suites?} \\
...
\textbf{How do baselines and contextual MABs perform across increasingly sophisticated threat regimes?}
...
\end{tikzpicture}
\vspace{-0.4cm}
\caption{Worst-case robustness is best reflected in the floor:
context-aware models retain substantially higher worst-case performance
than adversarial-first baselines at the default $2T$--$2T_b$ budget
(3-run horizons). \texttt{CPursuit} attains a high peak ($96.2\%$)
but a low floor ($37.2\%$), whereas \texttt{EXPNeuralUCB}'s floor
is $59.9\%$.}
\label{fig:floor}
\end{figure}
...
\begin{itemize}[leftmargin=2em]
  ...
  \emph{Do context-aware routing policies outperform baseline policies under stochastic disruption, and do they maintain this advantage under structured and adaptive threat regimes?}\\
  \textit{Yes.}
  ...
  \emph{Which model family degrades most gracefully and consistently defines the efficiency frontier across all threat scenarios?}\\
...
\textbf{What combinations of algorithm, allocator, and capacity semantics maximize routing efficiency and stability across deployment scenarios?}
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{RQ3a:} \textit{Does predictive (ARIMA-informed) context improve routing stability?} \\
  ...
  \item \textbf{RQ3b:} 
  \textit{Do larger replay-capacity scales ($T_b \rightarrow T$) improve routing robustness?} \\
  ...
  \item \textbf{RQ3c:} \textit{Is there a universal allocator strategy for entanglement routing under threat variation?} \\
  ...
  \item \textbf{RQ3d:} \textit{Can we map threat characteristics to stable deployment rules for entanglement routing?} \\
...
\end{tikzpicture}
\vspace{-0.4cm}
\caption{Win dominance under the default allocator (aggregated
across all configurations). \texttt{iCEpsilonGreedy} captures the
largest share of wins, with \texttt{CPursuit} second.}
...
\end{tikzpicture}
\vspace{-0.5cm}
\caption{Context vs.\ non-context efficiency across threat scenarios (evaluation corpus, paper-default allocator). Contextual (CMAB/iCMAB) models maintain higher Oracle-normalized efficiency than non-context EXP3 baselines under both $T$ and $T_b$ capacity semantics.}
...
\end{tikzpicture}
\vspace{-0.6cm}
\caption{Threat penalties relative to the Baseline scenario (evaluation corpus, paper-default allocator). Context-aware methods incur smaller and more stable penalties, while EXP3-family baselines are more vulnerable under adaptive (Adaptive/OnlineAdaptive) threats.}
...
\end{tikzpicture}
\vspace{-0.5cm}
\caption{Capacity--efficiency trade-offs in the hybrid evaluation corpus. Increasing capacity improves Baseline and Markov efficiency but can reduce robustness under Adaptive threats (capacity paradox).}
...
\end{tikzpicture}
\vspace{-0.5cm}
\caption{Threat-conditioned allocator performance (hybrid corpus, averaged over models). Thompson is strongest in Adaptive regimes, DynamicUCB is competitive under Adaptive/OnlineAdaptive, and Random underperforms.}
...
Values are the 3-run suite means from the hybrid evaluation corpus.}
\centering
...
\end{tikzpicture}
\vspace{-0.5cm}
\caption{Hybrid learning curves (illustrative): pursuit-based hybrids converge to high efficiency, while neural baselines plateau lower under sparse reward feedback.}
...
\end{tikzpicture}
\vspace{-0.5cm}
\caption{Hybrid evaluation corpus efficiency across threat scenarios. Pursuit-based hybrids maintain higher efficiency than neural and EXP3 baselines across all five regimes.}
