...
\end{axis}
\end{tikzpicture}
\caption[Fixed allocator tops the CPursuitNeuralUCB heatmap]{Fixed allocator yields the highest
efficiency for \texttt{CPursuitNeuralUCB} across threat scenarios, making allocator choice a first-class
robustness factor.}
\label{fig:heatmap}
\end{figure}
...
\footnotesize
\centering
\caption{Hyperparameter settings and their literature grounding.}
...
\footnotesize
\centering
\caption{Algorithm portfolio spans classical, adversarial, contextual, and predictive evaluation phases.}
...
\begin{table}[ht!]
\footnotesize
\caption{Allocator strategies determine how the fixed qubit budget is distributed across paths.}
...
\begin{table}[ht!]
\footnotesize
\caption{Context-aware baselines stay closest to deployment-viable efficiency under stochastic decoherence.}
...
\begin{table}[ht!]
\footnotesize
\caption{Predictive context improves robustness in the fixed 6K \texttt{Fixed} $T$-type $s{=}2$ deployment slice.}
\centering
...
\footnotesize
\centering
\caption{Replay capacity is non-monotonic: intermediate scaling helps, but larger scaling can regress under a fixed deployment anchor.}
...
\bottomrule
\end{tabular}
{\footnotesize{Bl=Baseline, Sh=Stochastic, Mk=Markov, Ag=Adaptive.}}
\end{table}
...
\footnotesize
\centering
\caption{Allocator choice materially changes robustness for \texttt{iCPursuitNeuralUCB}, with \texttt{Fixed} strongest overall.}
...
\vspace{0.2cm}
{\footnotesize \textbf{Note:} Random allocator is shown for completeness as a control baseline and is excluded from core deployment recommendations.}
\end{table}
...
\begin{table*}[ht!]
\centering
\caption{Cross-testbed performance is strongly topology-dependent, with \texttt{iCPursuitNeuralUCB} leading most testbeds while Paper~8 splits dominance from experiment wins.}
...
\vspace{0.2cm}
{\footnotesize \textbf{Legend:} N=Nodes, E=Edges, P=Paths, R=Runs, $s$=scale. Efficiency (\%) = (Model Avg Reward / Oracle Avg Reward) × 100. Gap = 100 - Efficiency. Exp.\ Winner reports experiment-level wins within each testbed corpus; $^\star$\textbf{bold} marks the testbed experiment winner.}
\end{table*}
...
\begin{table*}[ht!]
\centering
\caption{Pursuit-based hybrids define the strongest overall efficiency frontier across the internal framework and external testbeds.}
