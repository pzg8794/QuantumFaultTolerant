...
\begin{table*}[ht!]
\centering
\caption{Cross-testbed performance comparison using scenario-aggregated Oracle-normalized efficiency (\%) and scenario champions from \texttt{scenario\_winner}. Numeric columns are means across all five scenarios (\texttt{none}, \texttt{stochastic}, \texttt{markov}, \texttt{adaptive}, \texttt{onlineadaptive}) and 4 allocators. Papers 2, 7, and 12 use the full evaluation corpora aggregated across scales $s \in \{1.0,1.5,2.0\}$ (\texttt{cap\_type}=T) with 5 runs per setting; Paper 8 uses the current original paper-config slice at 1K/1K/1R, $s=1.0$, \texttt{cap\_type}=T.\devroop{Maybe have one more paper if you can.}}
