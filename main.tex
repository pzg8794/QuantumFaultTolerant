...
The primary contribution of this paper is a systematic threat-aware framework to evaluate contextual, adversarial, and hybrid bandit algorithms for joint path selection and qubit allocation in quantum networks, together with a family of pursuit--neural hybrid policies that outperform both adversarial-first and stochastic-only baselines. Unlike prior work that fixes allocator policy and replay semantics as background constants, our framework treats them as first-class experimental variables, enabling direct attribution of robustness to the algorithm--allocator--capacity triad across five threat regimes. We evaluate 13 algorithms across five threat scenarios while varying allocator policies and replay-capacity semantics.

Pursuit--neural hybrids (\texttt{CPursuitNeuralUCB}, \texttt{iCPursuitNeuralUCB}) emerge as the most robust family, outperforming non-contextual baselines by 18--24 percentage points in scenario-aggregated Oracle-normalized efficiency, sustaining worst-case performance floors above 85\% under stochastic threats, and remaining more stable under strategic adaptive attacks than adversarial-first EXP3-style designs. A key finding is a \emph{capacity paradox}: increasing replay capacity improves efficiency under structured (Markov) disruption yet induces efficiency collapses of 22--30 percentage points under adaptive adversaries---revealing that resource predictability, not bandwidth, is the dominant robustness bottleneck.
...
Quantum entanglement distribution is a foundational primitive of the quantum Internet, enabling applications ranging from quantum key distribution (QKD) to distributed quantum computing~\cite{kimble2008quantum,wehner2018quantum,pompili2021realization,wootters1982single,bennett1993teleporting}. Yet, reliable end-to-end entanglement is difficult to sustain: quantum states are fragile, entanglement generation and swapping are probabilistic, and performance degrades rapidly under decoherence and interference~\cite{briegel1998quantum,dahlberg2021netsquid,zukowski1993event}. In repeater-based architectures~\cite{briegel1998quantum,pompili2021realization}, entanglement swapping introduces stochastic waiting-time effects that compound along multi-hop routes~\cite{wang2019waiting}. These properties create a sequential decision problem in which a routing agent must repeatedly select among candidate paths while learning from noisy outcomes, making multi-armed bandits (MABs) a natural abstraction for online path selection and adaptation~\cite{lattimore2020bandit,bubeck2012regret}.
...
However, existing quantum routing research often evaluates algorithms under \emph{incompatible assumptions, metrics, or threat models}, complicating direct comparisons and obscuring deployment tradeoffs~\cite{liu2024qbgp,li2025multipath,chaudhary2023quantum}. This heterogeneity makes it difficult to attribute performance differences to the learning rule itself rather than to differences in topology scale, physics/noise modeling, allocation assumptions, or adversary behavior. Consequently, results may not translate into actionable deployment guidance when conditions drift or disruptions adapt. Two streams are especially prominent:

\begin{enumerate}
\item \textbf{Adversarial-first approaches} (\eg EXP3-family and adversarial-adaptive designs~\cite{auer2002nonstochastic,zimmert2019optimal,huang2024quantum}) assume worst-case link behavior and prioritize robustness under attack, often sacrificing stochastic efficiency.
...
To address these matched-threat evaluation gaps, we have developed a \emph{robust routing evaluation framework}
that compares adversarial-first (EXP3-family), stochastic/contextual-first
(CMAB/iCMAB-family), and \emph{hybrid pursuit--neural} models
(including EXPNeuralUCB~\cite{huang2024quantum}) under a unified threat taxonomy.
The overall decision flow is summarized in Fig.~\ref{fig:framework}.
\noindent Across the resulting evaluation corpora, we find that context-aware
pursuit--neural hybrids consistently offer the strongest
robustness--efficiency tradeoff, and that replay capacity exhibits a
threat-dependent \emph{capacity paradox} that can amplify vulnerability
under adaptive disruption.

\noindent Section~\ref{sec:studydesign} details the full experimental grid
(models, threats, allocators, and replay-capacity semantics).
