...
\end{tikzpicture}
\vspace{-0.4cm}
\caption{Global win share under the default allocator (aggregated
...
Replay capacity is not a \textquotedblleft more is better\textquotedblright knob. Under $T$-type anchoring (\Cref{tab:rq3b_capacity_scaling}), both pursuit-based hybrids
...
\item \textbf{\texttt{iCPursuitNeuralUCB} is the experiment winner on Papers 2, 7, and 12, but not on Paper 8.} It wins 94/300 experiments on Paper 2, 245/300 on Paper 7 (81.7\%), and 97/300 on Paper 12. On Paper 7, its dominance is overwhelming---no other model comes close. On Papers 2 and 12, the margin is tighter: Paper 2 sees a 4-way race (iCP: 94, G: 87, EXP: 81, CP: 38), while Paper 12 is a 2-way race (iCP: 97, EXP: 91). Paper 8 breaks this pattern: \texttt{EXPNeuralUCB} wins 10/20 allocator-scenario configurations, while \texttt{iCPursuitNeuralUCB} leads only 1/20 despite having the strongest aggregated efficiency.

\item \textbf{Scenario-aggregated ranking does not fully determine configuration-level winners.} In Paper 2, \texttt{iCPursuitNeuralUCB} has the highest aggregated efficiency (74.45\%) yet only wins 2/5 scenarios (none, markov). \texttt{CPursuitNeuralUCB} and \texttt{GNeuralUCB} each win 3/5 scenarios despite lower average efficiency, capturing adaptive, onlineadaptive, and stochastic. Paper 8 shows the same separation in a different form: \texttt{iCPursuitNeuralUCB} posts the highest aggregated efficiency (67.85\%), but \texttt{EXPNeuralUCB} wins the largest share of allocator-scenario configurations (10/20).
...
\item \textbf{Paper 8 adds a compact RL-routing regime with competitive but lower aggregate efficiencies than Papers 2 and 7.} Its model efficiencies span 61.4--67.9\%, placing it below Papers 2 and 7 but well above Paper 12. This extends the testbed-specific ranking story: the compact 20-node, 8-path Paper 8 network produces a more competitive EXP/iCP split than the strong iCP dominance seen on Paper 7.
...
& \texttt{EXPNeuralUCB} & 62.08 & 37.92 & 35.1 & \textbf{2/5$^\star$} \\
& \texttt{GNeuralUCB} & 61.81 & 38.19 & 37.1 & 0/5 \\
& \texttt{CPursuitNeuralUCB} & 61.65 & 38.35 & 37.8 & \textbf{2/5$^\star$} \\
\cline{2-6}
