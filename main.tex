...
\newcommand{\ChaudharyTestbedName}{Chaudhary\xspace}
\newcommand{\LiuTestbedName}{Liu\xspace}
\newcommand{\ClaytonTestbedName}{Clayton\xspace}
\newcommand{\JallowKhanTestbedName}{Jallow-Khan\xspace}
\newcommand{\ChaudharyTestbed}{Chaudhary \etal~\cite{chaudhary2023quantum}\xspace}
\newcommand{\LiuTestbed}{Liu \etal~\cite{liu2024qbgp}\xspace}
\newcommand{\ClaytonTestbed}{Clayton \etal~\cite{clayton2024quarc}\xspace}
\newcommand{\JallowKhanTestbed}{Jallow and Khan~\cite{jallowkhan2025adaptive}\xspace}
...
% % Add the straight feedbakc loop here
...
\begin{itemize}
    ...
    \item \descStep{Open Source Repository}{All source code, datasets, and experiment scripts are available in the public reproducibility repository at \url{https://github.com/pzg8794/quantum_project_hub}. Additional datalake-style artifacts and notebook execution resources are available through the shared Drive hub at \url{https://drive.google.com/drive/folders/0AK0VchnNyM-xUk9PVA}.}
...
\devroop{I feel like this should be within your related works section. And you can refactor your multi-armed bandit subsection into system model.}
...
\node[box] (context) {Candidate Paths\\+ Context};
\node[mainbox, right=of context] (agent) {Routing Agent\\(Bandit Policy)};
\node[box, right=of agent] (action) {Path Selection\\+ Qubit Allocation};

\node[box, below=1.0cm of action] (env) {Quantum Network\\+ Threat Regime};
\node[box, left=of env] (reward) {Reward / Feedback\\(success, fidelity, efficiency)};
...
\caption{System model for quantum entanglement routing. At each round, the routing agent observes candidate-path context, selects a path and qubit allocation, interacts with the quantum network under a given threat regime, and updates from the observed reward feedback.}
...
\noindent All quantitative results are computed from a curated experimental corpus generated by our evaluation framework. In total, we report \textit{7,890 model--scenario--configuration evaluations} across \textit{835 unique scenario--allocator--capacity--horizon settings}.
...
Our study addresses three core questions about stochastic decoherence impact, adversarial robustness, and deployment tradeoffs in algorithm--allocator--capacity selection:
...
yielding an average gap of approximately \textbf{24 pp} (see \cref{subsec:rq1_answer} and \cref{sec:SimulationResults}).
...
\emph{How does the performance of bandit-based routing strategies change as network disruptions evolve from stochastic noise to structured, adaptive adversarial interference?}
...
(see \cref{subsec:rq2_answer}). By comparison, representative classical baselines remain near \textbf{64--70\%} across the
...
\emph{How does algorithm choice, resource allocation strategy, and replay-capacity semantics interact to affect routing efficiency and stability in quantum entanglement routing?}
...
\emph{minimum variance} remains threat- and objective-dependent (see \cref{subsubsec:rq3a_answer}--\cref{subsec:rq3c_answer}).
...
\caption{Experimental design matrix linking configuration dimensions to the research questions.}
...
We evaluate five scenarios spanning the efficiency--security spectrum: natural stochastic decoherence, Markovian structure, and three grades of adaptive adversarial attacks (\RQTwo). This taxonomy enables controlled comparisons under matched interference regimes. Hyperparameter settings are summarized in \Cref{tab:setup-hyperparameters}.%
\cite{huang2024quantum}

\begin{table}[ht!]
\footnotesize
\centering
\caption{Hyperparameter settings and literature grounding.}
...
\begin{itemize}
  \item \small\texttt{Baseline} (\RQOne): Ideal 0\% failure rate. Sets the efficiency ceiling, isolating exploration costs from threat mitigation.

  \item \small\texttt{Stochastic} (\textbf{\RQOneText, RQ2a}): 6.25\% i.i.d.\ failure rate. Tests if specialized defenses are required under natural noise.%
  \cite{huang2024quantum}
...
% Algorithm Portfolio
% ----------------------------------------------------------------------------
\subsubsection{Algorithm portfolio (\RQTwo)}
...
\small
\centering
\caption{Algorithm progression by evaluation phase.}
...
\begin{table}[ht!]
\footnotesize
\caption{Qubit-budget distribution across candidate paths by allocator.}
...
\begin{itemize}
    \item \small\texttt{P1} (MAB baseline): 12 conditions. Classical models under stochastic noise to establish context-free baselines (\RQOne).
...
Results are organized by research question (\Cref{sec:research_questions}). We first assess whether baseline MABs remain deployment-viable under stochastic decoherence (\RQOneText; \cref{subsec:rq1_answer}), then evaluate robustness as threats become structured and adaptive (\RQTwoText; \cref{subsec:rq2_answer}), and finally analyze allocator--capacity interactions and deployment rules (\RQThreeText; \cref{subsubsec:rq3a_answer}--\cref{subsec:rq3c_answer}).
...
% ============================================================================
\subsection{\RQOneText: Impact of Stochasticity on Pure (Paper) Models}
...
efficiency, while weaker context-free or mis-specified variants degrade. \RQOneText{} establishes
fundamental viability \emph{before} introducing adaptive adversaries.

% ----------------------------------------------------------------------------
\subsubsection{Experimental Design}

For \RQOneText, we restrict analysis to the paper-model portfolio (EXP-family, CMAB, and iCMAB baselines)
...
\caption{\RQOneText{} performance under 6.25\% stochastic decoherence, aggregated across horizons, replay scales \(s \in \{1,1.5,2\}\), and both capacity semantics (T and Tb). Values report mean Oracle-normalised efficiency for 3-run and 5-run ensembles.}
...
Importantly, these \RQOneText{} conclusions already integrate the full replay scale sweep ($s \in \{1, 1.5, 2\}$)
and both capacity semantics ($T$, $T_b$); \RQThreeText{} later disaggregates capacity effects to diagnose the capacity paradox directly.
...
% ============================================================================
\subsection{\RQTwoText: Algorithm Robustness Across Threat Sophistication}
\label{subsec:rq2_answer}

\subsubsection{Experimental Design}

We evaluate the same paper-model portfolio from \RQOneText{} under three escalating threat environments:
...
\begin{table}[ht!]
\caption{Contextual and informed baselines remain more robust and stable than adversarial-first baselines under adversarial threats.}

\scriptsize
\centering
% \begin{tabular}{lcccc}
\begin{tabular}{@{}m{0.15\linewidth}>{\centering\arraybackslash}m{0.16\linewidth}>{\centering\arraybackslash}m{0.1\linewidth}>{\centering\arraybackslash}m{0.121\linewidth}>{\centering\arraybackslash}m{0.25\linewidth}@{}}
\toprule
...
\end{tikzpicture}
% \vspace{-0.6cm}
\caption{Context-aware methods absorb smaller, more stable threat penalties, while EXP3-family baselines are most exposed under adaptive threats. \shee{Is the line (next to each color box) in the legend required?}}
...
\noindent\textit{Supporting Question Answers:}
\begin{itemize}[nosep]
\item \small\texttt{\textbf{RQ2a}:} Yes on both counts. Under stochastic decoherence (6.25\% i.i.d.\ disruption), context-aware pursuit--neural models maintain 87--93\% Oracle-normalized efficiency, retaining the 15--25~pp advantage over classical baselines observed in \RQOneText{} and remaining above the 85\% deployment target. This advantage persists under structured and adaptive threats: across Markov, Adaptive, and OnlineAdaptive regimes, pursuit-based models sustain $\sim$89\% average efficiency with robustness floors of 65--73\%, while adversarial-first methods (EXP3-family) degrade to floors near 52--54.5\%, demonstrating that context-awareness provides superior robustness across all threat escalation levels.
...
% ============================================================================
\subsection{\RQThreeText: Deployment Optimization Strategies}
\label{subsec:rq3}
...
\RQThreeText{} characterizes which combinations are robust under threat escalation and which induce brittle failure modes.
...
\noindent
\RQThreeText{} is computed from the \textbf{hybrid evaluation corpus} (curated experimental suite with
...
% ----------------------------------------------------------------------------
\subsubsection{Answer to \RQThree}
\noindent
...
\vspace{0.1cm}
\scriptsize{\textbf{Note:} Random allocator shown for completeness as a control baseline. It is excluded from core deployment recommendations.}
\end{table}
...
\begin{axis}[
  ...
  grid=major,
  grid style={line width=.1pt, draw=gray!10},
  legend style={at={(0.45,-0.22)}, anchor=north},
  legend columns=-1,
  % legend pos=south east,
  % legend columns=2,
  legend style={font=\scriptsize}
...
To validate the generalizability and robustness of our proposed algorithms, we conducted cross-testbed evaluations by deploying our model portfolio on four independent quantum network simulation testbeds from prior work. These testbeds vary significantly in scale, network topology, noise modeling, and fidelity calculation approaches, providing a rigorous stress test for our context-aware pursuit-neural algorithms. This section presents: (1) performance comparison across the four external testbeds (\ChaudharyTestbedName, \LiuTestbedName, \ClaytonTestbedName, and \JallowKhanTestbedName), and (2) a consolidated comparison of model family performance (CMABs, iCMABs, Hybrid pursuit-neural, and EXP3-based models) across our internal evaluation suite.
...
\ChaudharyTestbedName, \LiuTestbedName, and \ClaytonTestbedName use the full multi-scale, 5-run evaluation corpora summarized above, while \JallowKhanTestbedName is currently represented by its original paper-config slice (1K/1K/1R, $s=1.0$, \texttt{cap\_type}=T).

\begin{table*}[ht!]
\centering
\caption{Cross-testbed performance comparison using scenario-aggregated Oracle-normalized efficiency (\%) and scenario champions from \texttt{scenario\_winner}. Numeric columns are means across all five scenarios (\texttt{none}, \texttt{stochastic}, \texttt{markov}, \texttt{adaptive}, \texttt{onlineadaptive}) and 4 allocators. \ChaudharyTestbedName, \LiuTestbedName, and \ClaytonTestbedName use the full evaluation corpora aggregated across scales $s \in \{1.0,1.5,2.0\}$ (\texttt{cap\_type}=T) with 5 runs per setting; \JallowKhanTestbedName uses the current original paper-config slice at 1K/1K/1R, $s=1.0$, \texttt{cap\_type}=T.}
...
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\ChaudharyTestbedName}\\ (\small 15N, 51E, 8P)\\ \small 4K/2K/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
...
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\LiuTestbedName}\\ (\small 50N, 141E, 15P)\\ \small 50/50/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
...
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\ClaytonTestbedName}\\ (\small 100N, 426E, 4P)\\ \small 1.5K/500/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
...
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\JallowKhanTestbedName}\\ (\small 20N, 19E, 8P)\\ \small 1K/1K/1R\\ \small 4 allocs, $s=1$, T}}
...
\vspace{0.2cm}
{\scriptsize \textbf{Legend:} N=Nodes, E=Edges, P=Paths, R=Runs, $s$=scale. Efficiency (\%) = (Model Avg Reward / Oracle Avg Reward) × 100. Gap = 100 - Efficiency. Numeric columns are means across all five scenarios and 4 allocators, using each testbed's corpus listed in the first column. Exp.\ Winner column shows experiment-level win count out of the available configurations for that corpus: 300 for \ChaudharyTestbedName/\LiuTestbedName/\ClaytonTestbedName (5 scenarios $\times$ 4 allocators $\times$ 3 scales $\times$ 5 experiments) and 20 for \JallowKhanTestbedName (5 scenarios $\times$ 4 allocators $\times$ 1 scale $\times$ 1 experiment). $^\star$\textbf{bold} = testbed experiment winner (highest win count). Scenario codes: \texttt{none}=baseline, \texttt{stochastic}=random failures, \texttt{markov}=oblivious adversarial, \texttt{adaptive}=feedback-driven, \texttt{onlineadaptive}=real-time adaptive.}
\end{table*}

\subsubsection{Key Observations from Cross-Testbed Validation}

\begin{enumerate}[nosep,leftmargin=2em]
\item \textbf{\texttt{iCPursuitNeuralUCB} is the cross-layer winner on \ChaudharyTestbedName, \LiuTestbedName, and \ClaytonTestbedName, while on \JallowKhanTestbedName it dominates only.} It wins 95/300 experiments on \ChaudharyTestbedName, 245/300 on \LiuTestbedName (81.7\%), and 97/300 on \ClaytonTestbedName. On \LiuTestbedName, its dominance is overwhelming---no other model comes close. On \ChaudharyTestbedName and \ClaytonTestbedName, the margin is tighter: \ChaudharyTestbedName sees a 4-way race (iCP: 95, G: 84, EXP: 83, CP: 38), while \ClaytonTestbedName is a 2-way race (iCP: 97, EXP: 91). \JallowKhanTestbedName breaks this pattern: \texttt{EXPNeuralUCB} wins 10/20 allocator-scenario configurations, while \texttt{iCPursuitNeuralUCB} leads only 1/20 despite having the strongest aggregated efficiency.

\item \textbf{Experiment threat ranking does not fully determine experiment-level dominance.} In the \ChaudharyTestbedName testbed, \texttt{iCPursuitNeuralUCB} has the highest dominance (scenario-aggregated efficiency, 74.45\%) yet wins only 2/5 threat scenarios (none, markov). \texttt{CPursuitNeuralUCB} and \texttt{GNeuralUCB} each win 3/5 threat scenarios despite lower dominance, capturing adaptive, onlineadaptive, and stochastic. The \JallowKhanTestbedName testbed shows the same separation in a different form: \texttt{iCPursuitNeuralUCB} has the highest dominance (scenario-aggregated efficiency, 67.85\%), while \texttt{EXPNeuralUCB} wins the largest share of allocator--threat-scenario configurations (10/20).

\item \textbf{The \ClaytonTestbedName testbed exhibits the broadest overlap in threat-scenario winners.} All four models win in at least 4 of 5 threat scenarios, with \texttt{markov} uniquely favoring \texttt{iCPursuitNeuralUCB} (51/60 experiment wins). This contrasts with \LiuTestbedName where threat-scenario championship is concentrated: \texttt{iCPursuitNeuralUCB} wins all 5 threat scenarios, with only \texttt{GNeuralUCB} and \texttt{EXPNeuralUCB} sharing \texttt{markov} experiment wins.

\item \textbf{The \JallowKhanTestbedName testbed adds a compact RL-routing regime with competitive but lower aggregate efficiencies than \ChaudharyTestbedName and \LiuTestbedName.} Its model efficiencies span 61.4--67.9\%, placing it below \ChaudharyTestbedName and \LiuTestbedName but well above \ClaytonTestbedName. This extends the testbed (default allocator) winner story: the compact 20-node, 8-path \JallowKhanTestbedName network splits \texttt{EXPNeuralUCB} experiment wins from \texttt{iCPursuitNeuralUCB} dominance, unlike the strong \texttt{iCPursuitNeuralUCB} cross-layer-winner pattern seen on \LiuTestbedName.

\item \textbf{\texttt{CPursuitNeuralUCB} never wins a threat scenario on \LiuTestbedName yet wins 3/5 threat scenarios on \ChaudharyTestbedName.} This testbed specificity highlights that winner structure is topology-dependent: the dense 50-node \LiuTestbedName network strongly favors \texttt{iCPursuitNeuralUCB} as the cross-layer winner, while the 15-node \ChaudharyTestbedName network enables broader competition.
\end{enumerate}

\subsection{Model Family Performance Comparison}

To provide a consolidated view of algorithm performance across model families, we aggregated results from our internal evaluation suite spanning CMABs, iCMABs, Hybrid pursuit-neural, and EXP3-based models. \Cref{tab:model_family_comparison} presents representative algorithms from each family under controlled conditions (4K base frames, 2K step frames, 5 runs, \texttt{cap\_type}=Tb), aggregated across all five scenarios using the \texttt{Default} allocator and available capacity scales. To enable direct comparison, we also include hybrid pursuit-neural results from the four external testbeds (\ChaudharyTestbedName, \LiuTestbedName, \ClaytonTestbedName, and \JallowKhanTestbedName) filtered to the same \texttt{Default} allocator (\texttt{cap\_type}=T, the only capacity semantics available in those corpora).

\begin{table*}[ht!]
\centering
\caption{Model family performance comparison: Representative algorithms from each bandit family on our internal evaluation framework (4K base/2K step/5 runs, \texttt{cap\_type}=Tb) plus hybrid pursuit-neural results on four external testbeds (\texttt{cap\_type}=T). All results use the \texttt{Default} allocator and aggregate across all five scenarios; \ChaudharyTestbedName, \LiuTestbedName, and \ClaytonTestbedName use scales $s\in\{1,1.5,2\}$, while \JallowKhanTestbedName uses its current 1K/1K/1R paper-config slice at $s=1.0$. Best per-family/testbed performance in \textbf{bold}.}
...
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\ChaudharyTestbedName}\\ \small(15N, 51E)\\ \small 4K/2K}}
...
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\LiuTestbedName}\\ \small(50N, 141E)\\ \small 50/50}}
...
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\ClaytonTestbedName}\\ \small(100N, 426E)\\ \small 1.5K/500}}
...
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\JallowKhanTestbedName}\\ \small(20N, 19E)\\ \small 1K/1K}}
...
\vspace{0.2cm}
{\scriptsize \textbf{Note:} Top section: internal evaluation corpora (CMABs, iCMABs, EXP3, Hybrid; runs=5, \texttt{cap\_type}=Tb, \texttt{Default} allocator, scales $s\in\{1,1.5,2\}$). Bottom section: external testbeds (\ChaudharyTestbedName, \LiuTestbedName, \ClaytonTestbedName, \JallowKhanTestbedName; \texttt{cap\_type}=T, \texttt{Default} allocator). \ChaudharyTestbedName, \LiuTestbedName, and \ClaytonTestbedName use runs=5 and scales $s\in\{1,1.5,2\}$; \JallowKhanTestbedName uses its current 1K/1K/1R paper-config slice at $s=1.0$. All results are aggregated across five scenarios. Gap = 100 - Efficiency. Floor = worst-case efficiency. Exp.\ Winner = experiment-level win count out of the available configurations for that corpus: 75 for \ChaudharyTestbedName/\LiuTestbedName/\ClaytonTestbedName and 5 for \JallowKhanTestbedName. $^\star$\textbf{bold} = family/testbed winner; ties are shown when win counts are equal.}
\end{table*}
...
\item \textbf{\texttt{iCPursuitNeuralUCB} is the strongest neural model by Oracle-relative gap reduction.} Across the Hybrid corpus and all four external testbeds (\ChaudharyTestbedName, \LiuTestbedName, \ClaytonTestbedName, and \JallowKhanTestbedName), it achieves the lowest aggregate gap over the five scenarios, and it retains that lead within each allocator slice. Its leadership is therefore stable to both topology and allocator variation.

\item \textbf{Efficiency degrades with testbed complexity.} Hybrid performance drops from 90.9\% on the internal corpus to 74.4\% (\ChaudharyTestbedName, 15N), 77.5\% (\LiuTestbedName, 50N), and 41.6\% (\ClaytonTestbedName, 100N). Crucially, low efficiency on the \ClaytonTestbedName testbed is \emph{not} caused by an intrinsic fidelity ceiling: the Oracle achieves an average reward of 0.87 ($\approx$87\% of the theoretical maximum), and individual model configurations reach up to 63\% of Oracle. The gap instead reflects the difficulty of bandit optimization on a 100-node, 4-path fusion topology---the large state space and constrained routing diversity make arm selection harder to learn within the 1.5K/500 frame regime, whereas the internal 4K/2K regime on a smaller topology gives the learner sufficient time to converge.
...
The cross-testbed validation presented in this section uses each testbed's native paper-config regime (\ChaudharyTestbedName: 4K/2K, \LiuTestbedName: 50/50, \ClaytonTestbedName: 1.5K/500, \JallowKhanTestbedName: 1K/1K/1R) while aggregating across allocators and scales. While this validates external generalizability under each testbed's original evaluation setup, it does not yet isolate differences under shared standardized run configurations. To address this, we are conducting a follow-up evaluation campaign that deploys all four external testbeds (\ChaudharyTestbedName, \LiuTestbedName, \ClaytonTestbedName, and \JallowKhanTestbedName) using our \textbf{standardized run configurations}:

\begin{itemize}[nosep]
\item \texttt{Base configuration:} 4K base frames, 2K frame steps, 3 runs (rapid convergence baseline)
\item \texttt{Stress testing configurations:} 5-run and 10-run sweeps to evaluate stability under extended learning horizons
\item \texttt{Fixed allocator:} \texttt{Default} or \texttt{Fixed} allocator across all testbeds to isolate algorithm effects
\item \texttt{Unified scenarios:} All five threat scenarios (Baseline, Stochastic, Markov, Adaptive, OnlineAdaptive) evaluated consistently
\end{itemize}
...
\item \textbf{Topology-algorithm interaction analysis} to determine whether algorithm rankings shift across sparse (\ChaudharyTestbedName: 15N, 51E), dense (\LiuTestbedName: 50N, 141E), and mid-scale (\ClaytonTestbedName: 100N, 426E) network structures
\end{enumerate}

Current evaluation-corpus evidence already shows strong testbed dependence in both efficiency and cross-layer winning structure, reinforcing the need for shared standardized run configurations across the external testbeds.

We report the initial standardized 4K/2K/5R sweep (config key \texttt{4000\_2000}) in \Cref{tab:testbed_comparison_standard_4000_2000} and the corresponding standardized \texttt{Default} allocator slice in \Cref{tab:external_default_standard_4000_2000}. \LiuTestbedName currently has partial standardized coverage (222/300 allocator--scale--experiment configs; 47/75 in the \texttt{Default} slice), and the Exp.\ Winner denominators reflect the available standardized configurations. Remaining stress sweeps (\eg 10-run) will be reported in a follow-up technical report and integrated into the reproducibility artifacts accompanying this paper.

% AUTO-GENERATED by tools/build_standardized_manuscript_tables.py
% Source: Validated_Logs/Master_Dataset_papers-4000_2000.csv
\begin{table*}[ht!]
\centering
...
\label{tab:testbed_comparison_standard_4000_2000}
\small
\begin{tabular}{l l c c c c l}
\toprule
\textbf{Testbed} & \textbf{Algorithm} & \textbf{Avg Reward} & \textbf{Regret} & \textbf{Efficiency (\%)} & \textbf{Gap (\%)} & \textbf{Exp. Winner} \\
...
& \texttt{EXPNeuralUCB} & 0.2810 & 583.2 & 71.25 & 28.75 & 83/300 \\
\cline{2-7}
& \cellcolor{findingBlue}\textit{Standardized 4K/2K slice} & \multicolumn{5}{l}{\cellcolor{findingBlue}\small 71.2--74.5\% eff. $^\star$Exp.\ Winner: iCPursuitNeuralUCB (95/300)} \\
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\LiuTestbedName}\\ (\small 50N, 141E, 15P)\\ \small 4K/2K/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
& \texttt{ORACLE} & 8.5977 & 0.0 & --- & --- & --- \\
& \texttt{iCPursuitNeuralUCB} & 6.7849 & 2730.7 & \textbf{78.10} & 21.90 & \textbf{155/222$^\star$} \\
& \texttt{EXPNeuralUCB} & 6.2643 & 10692.7 & 72.30 & 27.70 & 24/222 \\
& \texttt{CPursuitNeuralUCB} & 6.3630 & 10690.9 & 73.50 & 26.50 & 0/222 \\
& \texttt{GNeuralUCB} & 6.3630 & 10690.9 & 73.50 & 26.50 & 43/222 \\
\cline{2-7}
& \cellcolor{findingGreen}\textit{Standardized 4K/2K slice} & \multicolumn{5}{l}{\cellcolor{findingGreen}\small 72.3--78.1\% eff. $^\star$Exp.\ Winner: iCPursuitNeuralUCB (155/222)} \\
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\ClaytonTestbedName}\\ (\small 100N, 426E, 4P)\\ \small 4K/2K/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
& \texttt{ORACLE} & 0.8650 & 0.0 & --- & --- & --- \\
& \texttt{GNeuralUCB} & 0.3685 & 2889.1 & 42.36 & 57.64 & 54/300 \\
& \texttt{EXPNeuralUCB} & 0.3604 & 2950.1 & 41.42 & 58.58 & 75/300 \\
& \texttt{iCPursuitNeuralUCB} & 0.3715 & 2881.0 & \textbf{42.72} & 57.28 & \textbf{106/300$^\star$} \\
& \texttt{CPursuitNeuralUCB} & 0.3691 & 2885.2 & 42.43 & 57.57 & 65/300 \\
\cline{2-7}
& \cellcolor{findingOrange}\textit{Standardized 4K/2K slice} & \multicolumn{5}{l}{\cellcolor{findingOrange}\small 41.4--42.7\% eff. $^\star$Exp.\ Winner: iCPursuitNeuralUCB (106/300)} \\
\midrule
\multirow{7}{*}{\parbox{2.2cm}{\textbf{\JallowKhanTestbedName}\\ (\small 20N, 19E, 8P)\\ \small 4K/2K/5R\\ \small 4 allocs, $s\in\{1,1.5,2\}$, T}}
& \texttt{ORACLE} & 0.7904 & 0.0 & --- & --- & --- \\
& \texttt{iCPursuitNeuralUCB} & 0.5870 & 862.9 & \textbf{73.95} & 26.05 & 100/300 \\
& \texttt{EXPNeuralUCB} & 0.5295 & 1352.7 & 66.72 & 33.28 & 44/300 \\
& \texttt{GNeuralUCB} & 0.5825 & 933.1 & 73.39 & 26.61 & 49/300 \\
& \texttt{CPursuitNeuralUCB} & 0.5850 & 939.3 & 73.69 & 26.31 & \textbf{107/300$^\star$} \\
\cline{2-7}
& \cellcolor{findingBlue}\textit{Standardized 4K/2K slice} & \multicolumn{5}{l}{\cellcolor{findingBlue}\small 66.7--74.0\% eff. $^\star$Exp.\ Winner: CPursuitNeuralUCB (107/300)} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[ht!]
\centering
\caption{External testbed comparison under standardized 4K/2K/5R run configurations using the \texttt{Default} allocator (\texttt{cap\_type}=T). Columns match the external-testbed slice used in the model-family comparison: Avg Eff/Gaps are means across all five scenarios and scales $s \in \{1.0,1.5,2.0\}$; Floor is worst-case efficiency; Exp.\ Winner counts are out of the available standardized configurations.}
\label{tab:external_default_standard_4000_2000}
\small
\begin{tabular}{l l c c c l}
\toprule
\textbf{Testbed} & \textbf{Algorithm} & \textbf{Avg Eff (\%)} & \textbf{Gap (\%)} & \textbf{Floor (\%)} & \textbf{Exp. Winner} \\
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\ChaudharyTestbedName}\\ \small(Standard)\\ \small 4K/2K}}
& \texttt{iCPursuitNeuralUCB} & \textbf{74.44} & 25.56 & 54.4 & \textbf{25/75$^\star$} \\
& \texttt{CPursuitNeuralUCB} & 73.15 & 26.85 & 48.3 & 8/75 \\
& \texttt{GNeuralUCB} & 73.17 & 26.83 & 46.7 & 20/75 \\
& \texttt{EXPNeuralUCB} & 71.29 & 28.71 & 41.6 & 22/75 \\
\cline{2-6}
& \cellcolor{findingBlue}\textit{Standardized Default slice} & \multicolumn{4}{l}{\cellcolor{findingBlue}\small 71.3--74.4\% eff. $^\star$Exp.\ Winner: iCPursuitNeuralUCB (25/75)} \\
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\LiuTestbedName}\\ \small(Standard)\\ \small 4K/2K}}
& \texttt{iCPursuitNeuralUCB} & \textbf{78.25} & 21.75 & 47.8 & \textbf{36/47$^\star$} \\
& \texttt{CPursuitNeuralUCB} & 72.67 & 27.33 & 54.3 & 0/47 \\
& \texttt{GNeuralUCB} & 72.67 & 27.33 & 54.3 & 8/47 \\
& \texttt{EXPNeuralUCB} & 71.63 & 28.37 & 47.9 & 3/47 \\
\cline{2-6}
& \cellcolor{findingGreen}\textit{Standardized Default slice} & \multicolumn{4}{l}{\cellcolor{findingGreen}\small 71.6--78.2\% eff. $^\star$Exp.\ Winner: iCPursuitNeuralUCB (36/47)} \\
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\ClaytonTestbedName}\\ \small(Standard)\\ \small 4K/2K}}
& \texttt{iCPursuitNeuralUCB} & 42.80 & 57.20 & 24.7 & \textbf{27/75$^\star$} \\
& \texttt{CPursuitNeuralUCB} & \textbf{42.85} & 57.15 & 23.5 & 15/75 \\
& \texttt{GNeuralUCB} & 42.45 & 57.55 & 24.1 & 16/75 \\
& \texttt{EXPNeuralUCB} & 41.18 & 58.82 & 22.3 & 17/75 \\
\cline{2-6}
& \cellcolor{findingOrange}\textit{Standardized Default slice} & \multicolumn{4}{l}{\cellcolor{findingOrange}\small 41.2--42.9\% eff. $^\star$Exp.\ Winner: iCPursuitNeuralUCB (27/75)} \\
\midrule
\multirow{5}{*}{\parbox{2cm}{\textbf{\JallowKhanTestbedName}\\ \small(Standard)\\ \small 4K/2K}}
& \texttt{iCPursuitNeuralUCB} & \textbf{73.83} & 26.17 & 42.5 & 22/75 \\
& \texttt{CPursuitNeuralUCB} & 73.52 & 26.48 & 40.7 & \textbf{26/75$^\star$} \\
& \texttt{GNeuralUCB} & 73.72 & 26.28 & 46.1 & 16/75 \\
& \texttt{EXPNeuralUCB} & 67.74 & 32.26 & 22.7 & 11/75 \\
\cline{2-6}
& \cellcolor{findingBlue}\textit{Standardized Default slice} & \multicolumn{4}{l}{\cellcolor{findingBlue}\small 67.7--73.8\% eff. $^\star$Exp.\ Winner: CPursuitNeuralUCB (26/75)} \\
\bottomrule
\end{tabular}
\end{table*}
...
\end{tikzpicture}
% \vspace{-0.5cm}
\caption{Pursuit-based hybrids maintain higher efficiency than neural and EXP3 baselines across all five threat regimes.\shee{This legend is not self-explanatory. The graph has 2 colored portions visible, but the legend has 5 entries with no colors}}
...
Across the paper baselines (\RQOneText-\RQTwoText) and the Hybrid suite (\RQThreeText), \textbf{context-aware policies define the robustness frontier}. Contextual Pursuit models (\small\texttt{CPursuitNeuralUCB}, \small\texttt{iCPursuitNeuralUCB}) consistently outperform non-contextual baselines: under Stochastic noise, they exceed the EXP3-family by $\sim$18--24 pp and separate from neural non-context baselines by $\sim$6--12 pp (\RQOneText). Under Markov/Adaptive/OnlineAdaptive conditions, the gap persists: Pursuit remains near the high-$80\%$ band while EXP3-family baselines exhibit lower floors (\RQTwoText), reinforcing that topology/channel features are not an optimization detail but an \emph{architectural requirement} for robust routing.
...
As outlined in \cref{sec:testbed_comparison}, we are conducting follow-up evaluations that deploy all four external testbeds (\ChaudharyTestbedName, \LiuTestbedName, \ClaytonTestbedName, and \JallowKhanTestbedName) using our standardized run configurations (4K base/2K step frames, 3/5/10 runs, unified threat scenarios). This will enable direct efficiency comparisons across sparse (15-node), compact (20-node), dense (50-node), and mid-scale (100-node) topologies under aligned experimental conditions, testing whether pursuit-neural cross-layer winning structure persists and whether the capacity paradox manifests consistently across network structures.
...
The public reproducibility repository is available at \url{https://github.com/pzg8794/quantum_project_hub}, with supplementary datalake-style artifacts and executable notebooks available through the shared Drive hub at \url{https://drive.google.com/drive/folders/0AK0VchnNyM-xUk9PVA}.
