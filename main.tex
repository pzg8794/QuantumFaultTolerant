...
\title{Qubit Allocation in a Quantum Network using Stochastic Bandits}
...
\item \textbf{Contextual bandits} incorporate observable side information (e.g., topology features or load indicators) to improve decisions when context is predictive~\cite{chu2011contextual}.
...
\item \textbf{Adversarial bandits} guard against worst-case or non-stochastic reward sequences (e.g., EXP3 algorithm)~\cite{auer2002exp3}.
...
\begin{itemize}
\item $F_b$: \textbf{base per-run horizon} (frames)---the standard episode length before scaling (e.g., 4K, 6K, or 8K frames)
...
\begin{enumerate}
\item \textbf{ALLOCATOR:} Top-level policy (e.g., DynamicUCB)
...
\devroop{You have repeating supporting questions. Club them together}
...
\begin{itemize}[leftmargin=2em]
  ...
  \textit{Yes.}
  Under \small\texttt{Stochastic} decoherence, contextual and neural--contextual models retain the \textit{15--25 pp} gain observed in RQ1 and remain above the 85\% target.

  \item \small\texttt{\textbf{RQ2b}:}
  \emph{Under structured and reactive attacks, do adversarial-first defenses dominate?}\\
  \textit{No.}
  Pursuit-based models retain \textit{high averages ($\simeq$89\%)} and floors of \textit{65--73\%} across \small\texttt{Markov}, \small\texttt{Adaptive}, and \small\texttt{OnlineAdaptive} threats. Adversarial-first methods degrade to floors near \textit{52--54\%}.%
  \cite{auer2002nonstochastic}

  \item \small\texttt{\textbf{RQ2c}:}
  ...
  \item \small\texttt{\textbf{RQ2d}:}
  \emph{As threat sophistication increases (Baseline, \dots, OnlineAdaptive), which algorithms define the frontier?}\\
  % \small\texttt{iCPursuitNeuralUCB.}
  \small\texttt{iCPursuitNeuralUCB} defines the most favorable frontier, maintaining the highest overall average efficiency while preserving strong robustness floors.
\end{itemize}
...
\textbf{Supporting questions:}
\begin{itemize}[leftmargin=2em]
  \item \textbf{RQ2a:} \textit{Under natural noise, do context-aware models outperform classical baselines?} \\
  \emph{Yes}. The advantage from RQ1 persists under matched \textsc{Stochastic} conditions.

  \item \textbf{RQ2b:} \textit{Under structured and reactive attacks, do classical baselines remain competitive?} \\
  ...
  \item \textbf{RQ2c:} \textit{Which family degrades most gracefully as threat sophistication increases?} \\
  % \emph{Pursuit--neural.} 
  Pursuit--neural models preserve high mean efficiency across escalation, with degradation concentrated in worst-case \textsc{OnlineAdaptive}/\textsc{Markov} slices.

  \item \textbf{RQ2d:} \textit{Which algorithm best defines the frontier?} \\
  Across the validated suites, \small\texttt{iCPursuitNeuralUCB} is consistently on/near the top envelope for average efficiency while maintaining strong threat-wide robustness in many static configurations.
\end{itemize}
...
\devroop{format the figure. Labels and arrows are out of place a bit.}
...
Total capacity is 35 qubits, allocated per path by strategy (\Cref{tab:setup-allocators}).}
\begin{tikzpicture}[
  ...
  % ---- Paths (draw full paths; use slight offsets so they don't overlap) ----

  % P1: S -> R1 -> D (2-hop)  [more separation]
  \draw[path, blue]
    ($(S)+(0,0.09)$) -- node[above, font=\scriptsize] {$P_1$ (2-hop)} ($(R1)+(0,0.16)$) -- ($(D)+(0,0.16)$);

  % P2: S -> R2 -> D (2-hop)  [more separation]
  \draw[path, red]
    ($(S)+(0,-0.16)$) -- node[left, font=\scriptsize] {$P_2$ (2-hop)} ($(R2)+(0,-0.16)$) -- ($(D)+(0,-0.16)$);

  % P3: S -> R1 -> R2 -> D (3-hop)  [more separation]
  \draw[path, green!60!black]
    ($(S)+(0.22,0.22)$) -- ($(R1)+(0.22,0.26)$)
    -- node[right, font=\scriptsize] {$P_3$ (3-hop)} ($(R2)+(0.22,-0.22)$)
    -- ($(D)+(0.32,-0.14)$);

  % P4: S -> R2 -> R1 -> D (3-hop)  [more separation]
  \draw[path, orange]
    ($(S)+(-0.32,-0.14)$) -- ($(R2)+(-0.22,-0.22)$)
    -- node[left, font=\scriptsize] {$P_4$ (3-hop)} ($(R1)+(-0.16,0.07)$)
    -- ($(D)+(-0.17,0.07)$);

  % Annotations
  \node[below=0.3cm of S, font=\scriptsize] {Total: 35 qubits};
  \node[below=0.3cm of D, font=\scriptsize, align=center]
    {Example splits: \\ Fixed $(8,10,8,9)$, Thompson $(9,9,9,8)$};
\end{tikzpicture}
...
\begin{itemize}
  ...
  \item \small\texttt{Markov} (\textbf{RQ2b}): 25\% attack rate using a 4-state transition process. Models state-dependent disruption.%
  \cite{expneural2024}

  \item \small\texttt{Adaptive} (\textbf{RQ2b}): 25\% attack rate targeting high-usage paths over a sliding window ($w=50$). Models reactive targeting based on medium-term behavior.%
  \cite{huang2024quantum,expneural2024}

  \item \small\texttt{OnlineAdaptive} (\textbf{RQ2b}): 25\% attack rate with exponential memory decay ($\gamma=0.97$) and softmax targeting. Models continuous policy adaptation at the highest threat level.%
  \cite{huang2024quantum,expneural2024}
\end{itemize}
...
Allocator-comparison and full hybrid deployment rules (e.g., \small\texttt{Fixed/Thompson/DynamicUCB/Random})
...
Corpus results show a clear separation under stochastic decoherence: a small top tier remains
...
\item \textit{\textbf{RQ2c}:} \small\texttt{iCEpsilonGreedy} provides the best stability-floor profile (CV 3.6\%, floor 81\%), whereas \small\texttt{EXPNeuralUCB} shows the strongest fragility (CV 16.5\%, floor 18\%).
\end{itemize}
...
effective in specific regimes (e.g., Adaptive) but can underperform severely when mismatched
...
Predictive context is currently instantiated via an ARIMA warm‑up procedure. More expressive temporal models (e.g., RNN/Transformer families) and calibrated uncertainty mechanisms are not yet evaluated, and their sensitivity, stability, and failure modes under reactive adversaries remain open.
